...
void Transform::Jump(float height, float deltaTime, float & j_elapsed)
{
	// cycle is time
	float cycle = 4 * glm::sqrt(2 * height / gravity);
	float frequency = 1 / cycle;
	float amplitude = height * 2;
	j_elapsed += deltaTime;

	float width = cycle * glm::length(velocity) / 2.0f;

	position.y = height * glm::sin(2 * glm::pi<float>() * j_elapsed / cycle) + 2;
	position += velocity * deltaTime;

	if (j_elapsed > (cycle / 2.0f)) {
		j_elapsed = 0.0f;
	}
